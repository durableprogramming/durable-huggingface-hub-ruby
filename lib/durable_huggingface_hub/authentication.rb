  ...
  module Authentication
    ...
    def self.login(token: nil, add_to_git_credential: false)
      ...
      begin
        ...
        raise DurableHuggingfaceHubError, "Login failed: #{e.message}"
      end
      ...
      store_git_credential(token) if add_to_git_credential
    ...
    def self.obtain_token(token)
      ...
      unless $stdin.respond_to?(:tty?) && $stdin.tty?
        raise DurableHuggingfaceHubError, "No token provided and not running interactively. " \
    ...
    def self.prompt_for_token
      ...
      begin
        token = if $stdin.respond_to?(:noecho) && $stdin.isatty
        ...
        puts "\nLogin cancelled"
        raise DurableHuggingfaceHubError, "Login cancelled by user"
      end

      raise DurableHuggingfaceHubError, "Token entry cancelled or empty" if token.nil? || token.empty?
