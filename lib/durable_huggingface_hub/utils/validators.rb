  ...
  module Utils
    ...
    module Validators
      ...
      def self.validate_repo_id(repo_id, repo_type: nil)
        raise ValidationError.new("repo_id", "Repository ID cannot be empty") if repo_id.nil?

        unless repo_id.is_a?(String)
          raise ValidationError.new("repo_id", "Repository ID must be a string, not #{repo_id.class}: '#{repo_id}'")
        end

        raise ValidationError.new("repo_id", "Repository ID cannot be empty") if repo_id.empty?
        ...
        if repo_id.count("/") > 1
          raise ValidationError.new("repo_id",
                                    "Repository ID must be in format " \
                                    "'repo_name' or 'namespace/repo_name': '#{repo_id}'")
        end
        ...
        unless repo_id.match?(%r{\A(\b[\w\-.]+\b/)?\b[\w\-.]{1,96}\b\z})
          raise ValidationError.new("repo_id",
                                    "Repository ID must use alphanumeric chars, '-', '_' or '.'. " \
                                    "The name cannot start or end with '-' or '.' " \
                                    "and the maximum length is 96: '#{repo_id}'")
        end
      ...
      def self.validate_revision(revision)
        raise ValidationError.new("revision", "Revision cannot be empty") if revision.nil? || revision.empty?

        # Check length (reasonable max for branch/tag names)
        raise ValidationError.new("revision", "Revision name is too long") if revision.length > 255

        # If it looks like a commit SHA (40 hex chars), validate that
        return revision if revision.match?(Constants::REGEX_COMMIT_OID)

        # For branch/tag names, allow alphanumeric, hyphen, underscore, dot, slash
        unless revision.match?(%r{\A[a-zA-Z0-9._/-]+\z})
      ...
      def self.validate_filename(filename)
        raise ValidationError.new("filename", "Filename cannot be empty") if filename.nil? || filename.empty?

        # Disallow absolute paths
        raise ValidationError.new("filename", "Filename cannot be an absolute path") if filename.start_with?("/")
        ...
        # Disallow null bytes
        raise ValidationError.new("filename", "Filename cannot contain null bytes") if filename.include?("\0")
      ...
      def self.require_non_nil(value, name)
        raise ValidationError.new(name, "#{name} is required and cannot be nil") if value.nil?
